...
plotdat <- expand.grid(
  ...
  filter(model %in% c('Diminishing', 'Cubic', 'Quadratic'))


# cc <- scales::seq_gradient_pal("blues")(seq(0,1,length.out=4))

gg <- ggplot(plotdat) +
  geom_line(aes(expectedLifetimes, y = modelPrediction, group=cohortSize, color=cohortSize), size=1) +
  theme_minimal() +
  facet_grid(~model) +
  theme(
    legend.position="top",
  ...
  scale_color_continuous(name = 'Cohort Size', breaks = c(4, 8, 16)) +
